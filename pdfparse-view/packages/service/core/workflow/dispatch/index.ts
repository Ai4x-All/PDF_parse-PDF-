...
export async function dispatchWorkFlow(data: Props): Promise<DispatchFlowResponse> {
  ...
  function pushStore(
    ...
    // console.log('responseData~~~~~', responseData[textOutput]);
  ...
  function nodeOutput(
    ...
    node.outputs.forEach((outputItem) => {
      ...
      let entry: any = { key: outputItem.key, value: outputItem.value }; 
      // console.log(`output value~~~~~~~~~~~ ${outputItem.key} = ${outputItem.value}`);
      if (outputItem.key === 'fields') {  
          const parsedValue = JSON.parse(outputItem.value); 
          entry.parsedValue = parsedValue; 
          console.log(`Parsed fields object~~~~~~~~~~~~~~~~~~~~:`, parsedValue);  
      } else { }  
      // if (outputItem.key === 'system_text') {  
      //     // const parsedValue = JSON.parse(outputItem.value);  
      //     entry.system_text = outputItem.value;
      //     // console.log(`system_text~~~~~~~~~~~~~~~~~~~:`, outputItem.value);  
      // } else { }  
      logData.push(entry)
    });
