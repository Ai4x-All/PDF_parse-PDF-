...
type useChatStoreType = OutLinkChatAuthProps &
  ChatProviderProps & {
    videofig?:any, setVideofig?: any;
    prevplayingChatId?:any;
    setVideoWidth?:any;
    setPrevPlayingChatId:any;
    welcomeText: string;
...
export const ChatBoxContext = createContext<useChatStoreType>({
  ...
  playAudioByTextnormal: function ({
    text,
    buffer
  }: {
    text: string;
  ...
  cancelAudionormal: function (): void {
    throw new Error('Function not implemented.');
  },
...
const Provider = ({
  ...
  const [prevplayingChatId, setPrevPlayingChatId] = useState<any>(null);
  const [videofig, setVideofig] = useState<any>({  
    width: 260,  
    height: 0,  
    right: 20,  
    bottom: 0,  
  ...
  const value: useChatStoreType = {
    ...props,
    videofig, setVideofig,
    prevplayingChatId,
    setPrevPlayingChatId,
    shareId,
    outLinkUid,
    teamId,
    teamToken,
