...
import { Box, Flex, Checkbox, Button, Drawer, DrawerOverlay, DrawerContent, DrawerCloseButton, DrawerHeader, useDisclosure } from '@chakra-ui/react';
...
import TwoDpeople from './components/TwoDpeople'
...
//   showpeople: { isVisible: false, message: 'No data' },
// }
...
const ChatBox = (
  ...
  const [lastStable, setLastStable] = useState<any>(false);
  ...
  const [zimu, setZimu] = useState<any>('');
  ...
  const [peopleheight, setPeopleheight] = useState<any>('0');
  const[peopleplay, setPeopleplay] = useState<any>(false);
  return (
            ...
            (
              (ischeckbox) ? (
                ...
                <>
                  ...
                  <Box position={'absolute'} width={'356px'} height={'300px'}
                    bottom={'-28px'} right={'0'} zIndex={99}>
                    <TwoDpeople
                    lastStable={lastStable}
                    setLastStable={setLastStable}
                    zimu={zimu}
                      setPeopleplay={setPeopleplay}
        ...
        onStartChat && chatStarted && active && appId && (
          <Box display={'flex'} justifyContent={'center'}>
            ...
            />
            {
              (showpeople === '1') ? (
                <>
                  {(!ischeckbox) && (<Button height={'80%'} m={'auto 0'} bg={'#A7BAEB'} onClick={onOpen}>
                    <MdFormatAlignRight size={'20px'} />
                  </Button>)}
                  <Checkbox isChecked={ischeckbox} onChange={handleCheckboxChange} ml={'20px'} color={'white'} size={'lg'} >
                    小窗模式
                  </Checkbox>
                  <Drawer
                    isOpen={isOpen}
                  ...
                  >
                    <DrawerOverlay />
                    <DrawerContent borderRadius='20px 0 0 20px ' overflowY='auto' maxHeight={'100vh'}>
                      {/* <DrawerCloseButton /> */}
                      {/* <DrawerHeader>聊天记录</DrawerHeader> */}
                      <Box id="chat-container" maxW={['100%', '92%']} h={'100%'} mx={'auto'}>
                        {showEmpty && <Empty />}
