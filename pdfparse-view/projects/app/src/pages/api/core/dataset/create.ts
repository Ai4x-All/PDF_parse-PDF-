...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 解构 req.body 以获取所需参数
    const { parentId, name, type, avatar, vectorModel, agentModel } = req.body;

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/create`;

    // 使用 axios 发送 POST 请求，并将参数传递到目标 API
    const response = await axios.post(
      targetUrl,
      { parentId, name, type, avatar, vectorModel, agentModel },
      {
        headers: {
