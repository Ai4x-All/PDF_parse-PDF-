...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 解构 req.query 以获取 collectionId
    const { collectionId } = req.query;

    // 检查 collectionId 是否存在
    if (!collectionId) {
      return res.status(400).json({ message: 'collectionId is required' });
    }

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/collection/read`;

    // 使用 axios 发送 GET 请求，并将 collectionId 作为查询参数传递到目标 API
