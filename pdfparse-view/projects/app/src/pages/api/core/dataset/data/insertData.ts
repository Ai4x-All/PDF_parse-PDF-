...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 解构 req.body 以获取所需参数
    const { collectionId, q, a, indexes } = req.body;

    // 检查必要参数
    if (!q || !collectionId) {
      return res.status(400).json({ message: 'Missing required parameters: q or collectionId' });
    }

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/data/insertData`;

    // 使用 axios 发送 POST 请求，并将参数传递到目标 API
    const response = await axios.post(
      targetUrl,
      { collectionId, q, a, indexes },
      {
        headers: {
