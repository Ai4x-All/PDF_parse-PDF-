...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!collectionId) {
      return res.status(400).json({ message: 'Missing collection ID' });
    }

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/collection/delete`;

    // 使用 axios 发送 DELETE 请求
    const response = await axios.delete(targetUrl, {
      params: { id: collectionId }, // 将 collectionId 作为查询参数传递
      headers: {
