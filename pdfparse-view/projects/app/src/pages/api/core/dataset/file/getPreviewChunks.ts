...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 解构 req.body 以获取所需参数
    const { type, sourceId, chunkSize, customSplitChar, overlapRatio, selector, isQAImport } = req.body;
    ...
    if (chunkSize > 30000) {
      return res.status(400).json({ message: 'chunkSize is too large, should be less than 30000' });
    }

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/file/getPreviewChunks`;

    // 使用 axios 发送 POST 请求，并将参数传递到目标 API
    const response = await axios.post(
      targetUrl,
      { type, sourceId, chunkSize, customSplitChar, overlapRatio, selector, isQAImport },
      {
        headers: {
