...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 检查必要参数
    if (!datasetId || !vectorModel) {
      return res.status(400).json({ message: 'datasetId or vectorModel is required' });
    }

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/training/rebuildEmbedding`;
