...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 解构 req.body
    const { link, trainingType, chunkSize, chunkSplitter, qaPrompt, ...body } = req.body;

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/collection/create/link`;

    // 使用 axios 发送 POST 请求，传递解构后的 req.body
    const response = await axios.post(targetUrl, {
      link,          // 将 link 传递到目标 API
