...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 如果 datasetId 或 text 不存在，返回错误
    if (!datasetId || !text) {
      return res.status(400).json({ message: 'Missing required parameters: datasetId or text' });
    }

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/dataset/searchTest`;
