...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 解构 req.body 以获取所需参数
    const { pageNum, pageSize, appId, dateStart, dateEnd } = req.body;

    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/app/getChatLogs`;

    // 使用 axios 发送 POST 请求，并将参数传递到目标 API
    const response = await axios.post(
      targetUrl,
      { pageNum, pageSize, appId, dateStart, dateEnd },
      {
        headers: {
