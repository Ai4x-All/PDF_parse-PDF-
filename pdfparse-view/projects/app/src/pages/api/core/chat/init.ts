...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 解构 req.query 以获取 dataId
    const { appId,chatId,loadCustomFeedbacks } = req.query;
    ...
    // 构造目标 API 的 URL
    const targetUrl = `https://www.xiaoruiai.com:23000/api/core/chat/init`;

    // 使用 axios 发送 GET 请求
    const response = await axios.get(targetUrl, {
      params: { appId,chatId,loadCustomFeedbacks },
      headers: {
