...
const Radio = ({ previewUrlprops, setPdfUrl, setMddata }:
  { previewUrlprops: any, setPdfUrl: any, setMddata: any }) => {
  const { goToNext, positioninfo, setPositioninfo, sources, setSources, previewUrl, setPreviewUrl, pdfresponse } = useContextSelector(DatasetImportContext, (v) => v);
  useEffect(() => {
          ...
          <div className="wrap">
            {
              previewUrl.map((item: any, index) => {
                return (
                  <>
                    <input hidden autoFocus={false} className={`rd-${index + 1}`}
                      name="radio" id={`rd-${index + 1}`} type="radio" />
                    <label autoFocus={false} style={{ zIndex: 1 }} onClick={() => handlePreview(item, index)} className="label" htmlFor={`rd-${index + 1}`} >
    ...
    <Tabbar>
      ...
      <div className="radio-inputs">
        ...
        <label className="radio">
          <input defaultChecked name="radio" type="radio" onFocus={(e) => e.target.blur()} />
...
const Tabbartwocpt = () => {
  const { filereferenceCurentPage, setfilereferenceCurentPage } = useContextSelector(DatasetImportContext, (c) => c);
  const handeltabbarMD = () => {
    setfilereferenceCurentPage([{
      name: 'Markdown结果',
      uuid: 0
    }])
  }
  const handeltabbarIMG = () => {
    setfilereferenceCurentPage([{
      name: '图片',
      uuid: 1
    }])
  }
  const handeltabbarTBL = () => {
    setfilereferenceCurentPage([{
      name: '表格',
      uuid: 2
    }])
  }
  const handeltabbarTXT = () => {
    setfilereferenceCurentPage([{
      name: '文本',
      uuid: 3
    }])
  }
  const handeltabbarJSON = () => {
    setfilereferenceCurentPage([{
      name: '原始JSON',
      uuid: 4
    }])
  ...
  return (
    <Tabbartwo>
      <div className="radio-inputs">
        <label className="radio">
          <input type="radio" name="radio" defaultChecked onClick={handeltabbarMD} />
          <span className="name">Markdown结果</span>
        </label>
        <label className="radio">
          <input type="radio" name="radio" onClick={handeltabbarIMG} />
          <span className="name">图片</span>
        </label>
        <label className="radio">
          <input type="radio" name="radio" onClick={handeltabbarTBL} />
          <span className="name">表格</span>
        </label>
        <label className="radio">
          <input type="radio" name="radio" onClick={handeltabbarTXT} />
          <span className="name">文本</span>
        </label>
        <label className="radio">
          <input type="radio" name="radio" onClick={handeltabbarJSON} />
...
export default function FileReference() {
  ...
  return (
    <>
      ...
      < Radio previewUrlprops={previewUrl} setMddata={setMddata} setPdfUrl={setPdfUrl} />
    </>
  )
}
