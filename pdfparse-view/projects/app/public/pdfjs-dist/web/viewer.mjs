...
class AnnotationLayerBuilder {
  ...
  async render(viewport, intent = "display") {
    ...
    if (annotations.length === 0) {
      // [Custom] always turn the annotationLayer on
      // this.hide();
      return;
    }
    ...
    // [custom-annotationLayer]
    // console.log('test-custom-annotationLayer', window?.PDFViewerApplication?.pdfViewer?._pages?.length)
    window?.PDFViewerApplication?.pdfViewer?._pages?.forEach((i,index) => {
        window.parent.postMessage({pageNum: index}, '*');
    })
...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    // console.log('test-appConfig.sidebar', appConfig.sidebar)
  ...
  async run(config) {
    ...
    fileInput.addEventListener("change", function (evt) {
      ...
      console.log('test-file-open',files)
  ...
  async open(args) {
    ...
    return loadingTask.promise.then(pdfDocument => {
      // [Custom] desc: aƒdd loaded listen
  ...
  load(pdfDocument) {
    ...
    firstPagePromise.then(pdfPage => {
      ...
      Promise.all([_ui_utils_js__WEBPACK_IMPORTED_MODULE_0__.animationStarted, storedPromise, pageLayoutPromise, pageModePromise, openActionPromise]).then(async ([timeStamp, stored, pageLayout, pageMode, openAction]) => {
        ...
        let scrollMode = 3 || _app_options_js__WEBPACK_IMPORTED_MODULE_2__.AppOptions.get("scrollModeOnLoad");
        let spreadMode = _app_options_js__WEBPACK_IMPORTED_MODULE_2__.AppOptions.get("spreadModeOnLoad");
        console.log('test-page-mode', sidebarView, scrollMode, spreadMode, stored.page)
        if (stored?.page && viewOnLoad !== ViewOnLoad.INITIAL) {
          // [custom]
          // hash = `page=${stored.page}&zoom=${zoom || stored.zoom},` + `${stored.scrollLeft},${stored.scrollTop}`;
          hash = `page=${0}&zoom=${zoom || stored.zoom},` + `${stored.scrollLeft},${stored.scrollTop}`;
        ...
        // TIP： 屏蔽上述scrollMode逻辑强制设置scrollMode 为3
        // custom scrollMode
        // console.log('test-dd',  evt.pageNumber)
        scrollMode = 2
        spreadMode = 0
  ...
  async _initializeMetadata(pdfDocument) {
    ...
    // [Custom] desc: use title from postMessage
    // const odlPdfTitle = document.getElementById("odl-pdf-title");
    // odlPdfTitle.innerText = this._contentDispositionFilename || this._title;
    // odlPdfTitle.setAttribute('title', this._contentDispositionFilename || this._title);
...
class PDFDocumentProperties {
  ...
  constructor({
    ...
    eventBus._on("pagechanging", evt => {
      ...
      // console.log('test-number-message-send', evt.pageNumber,window.renderExtractLayer);
      window.parent.postMessage({pageNum: evt.pageNumber}, '*');
...
// const SYLLABLES_REG_EXP =  /[\u4e00-\u9fff\u3400-\u4dbf\u20000-\u2a6df\u2a700-\u2b73f\u2b740-\u2b81f\u2b820-\u2ceaf\u2ceb0-\u2ebe0\u30000-\u3134f\u2e80-\u2eff\u3000-\u303f\uf900-\ufaff\uff00-\uffef]+/g;
const SYLLABLES_LENGTHS = new Map();
const FIRST_CHAR_SYLLABLES_REG_EXP = "[\\u1100-\\u1112\\ud7a4-\\ud7af\\ud84a\\ud84c\\ud850\\ud854\\ud857\\ud85f]";
// const FIRST_CHAR_CHINESE_REG_EXP = "[\\u4e00-\\u9fff\\u3400-\\u4dbf\\u20000-\\u2a6df\\u2a700-\\u2b73f\\u2b740-\\u2b81f\\u2b820-\\u2ceaf\\u2ceb0-\\u2ebe0\\u30000-\\u3134f\\u2e80-\\u2eff\\u3000-\\u303f\\uf900-\\ufaff\\uff00-\\uffef]";
...
class PDFPageView {
  ...
  async #renderAnnotationEditorLayer() {
    ...
    // [Custom] annotationLayer有时候会重建，所以多一层postMesseage保证朦层渲染
    window.parent.postMessage({pageNum: this.id - 1}, '*');
    }
  }
  ...
  async draw() {
    ...
    const resultPromise = renderTask.promise.then(async () => {
      ...
      if (!this.annotationEditorLayer) {
        // console.log('test-drawer', this.drawLayer.getDrawLayer())
...
class PDFSidebar {
  ...
  setInitialView(view = _ui_utils_js__WEBPACK_IMPORTED_MODULE_0__.SidebarView.NONE) {
    ...
    // [Custom] desc：  do not open the siderBar when init the view
...
class PDFThumbnailView {
  constructor({
    ...
    const odlImageNum = document.createElement("div");
    odlImageNum.classList.add(window.odlCustomSiderDomList.odl_imageNum);
    // console.log('test-this', this)
    odlImageNum.innerText =  this.id
    anchor.append(odlImageNum)
  }
  #updateDims() {
    const {
      width,
    ...
    // console.log('test-canvas', this.canvasWidth, this.canvasHeight, THUMBNAIL_WIDTH);
...
const ODL_THUMBNAIL_SELECTED_CLASS = "odl_selected";
class PDFThumbnailViewer {
  ...
  scrollThumbnailIntoView(pageNumber) {
    ...
    if (pageNumber !== this._currentPageNumber) {
      ...
      prevThumbnailView.div.parentNode.classList.remove(ODL_THUMBNAIL_SELECTED_CLASS)
      thumbnailView.div.classList.add(THUMBNAIL_SELECTED_CLASS);
      thumbnailView.div.parentNode.classList.add(ODL_THUMBNAIL_SELECTED_CLASS)
    }
    const {
      first,
      last,
  ...
  forceRendering() {
    ...
    // 有缓存 thumbView null 所以返回false， 不强制更新
    if (thumbView) {
...
class PDFViewer {
  ...
  set currentPageNumber(val) {
    ...
    // console.log('test-setCurrent-1')
    // [custom] desc: 上面按钮切换pageNumber
  ...
  scrollPageIntoView({
    ...
    if (this.isInPresentationMode || !destArray) {
      // [custom] 图片视图切换 pageNumber
      // console.log('test-setCurrent-2', pageNumber, destArray, this.isInPresentationMode)
  ...
  _getVisiblePages() {
    ...
    // console.log('test-views',views,horizontal, rtl  )
...
class Toolbar {
  ...
  #updateUIState(resetNumPages = false) {
    ...
    if (this.hasPageLabels) {
      ...
      // console.log('test-pageNumber-dd', pageNumber)
      // [custome] 比page Changing 更精确
      window.parent.postMessage({pageNumDetail: pageNumber}, '*');
    ...
    if(opts?.odlScale) {
      opts.odlScale.innerText =  `${ Math.round(pageScale * 10000) / 100}%`
    }
  }
...
class ViewHistory {
  ...
  async _readFromStorage() {
    // 屏蔽本地存储的pageNum字段 [custom]
    const history = localStorage.getItem("pdfjs.history");
...
function getViewerConfiguration() {
  const odlCustomToolBarDomList = window.getOdlViewDom ? window.getOdlViewDom(window.odlIdDomToolbarList) : {};
  const odlCustomSiderDomList = window.getOdlViewDom ? window.getOdlViewDom(window.odlCustomSiderDomList) : {};
